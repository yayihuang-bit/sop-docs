...
- **各分類有自己的主題色**：遊戲類教學＝紫、營運活動教學＝棕、競品指南＝綠、Git使用教學＝藍綠、包鑽商城＝紫紅、其他文件＝藍灰，卡片邊框、按鈕、標題文字都統一套色
...
├── docs/
│   ├── index.md                  # 首頁：常用連結 + 六大分類卡片導覽
│   ├── 連結總表.md               # 內部工具頁，不排進 nav，用來列出 links 清單方便挑選常用連結預設值
│   ├── 遊戲上線工作_索引.md／遊戲上線SOP.md／遊戲調整流程.md
│   ├── 營運_索引.md／營運企劃執行工作大綱.md／營運活動執行流程.md／甘特圖轉換工具.md／文案調整.md／營運文案寫法.md／營運活動基本設定.md／換皮資訊_索引.md／代言人更新流程.md
...
│   ├── 包鑽商城.md（內容 🟡 待補充骨架）  # 包鑽商城分類（第 5 個大分類），單頁、nav 扁平化
│   ├── 其他文件教學_索引.md／BUG回報方式.md／友善遊戲.md  # 其他文件分類（第 6 個大分類，nav 顯示名稱是「其他文件」，索引檔名還留著舊名 `其他文件教學_索引.md` 沒改，跟 `營運_索引.md` 檔名不完全對應分類全名是同樣的慣例），有 2 頁以上後改回標準的「首頁＋子頁」巢狀結構
...
nav:                     # 見檔案內容，目前是「首頁 / 遊戲類教學 / 營運活動教學 / 競品指南 / Git使用教學 / 包鑽商城 / 其他文件」六大分類
...
`docs/連結總表.md` 有目前所有 `links.*` 代號跟中文說明的對照，忘記代號叫什麼可以去那邊查。**同一份試算表常常有好幾個口語俗稱**（例如 `update_plan_sheet` 也被叫「包你發大表」「大表」，`campaign_doc_sheet` 也被叫「營運文件排程表」「營運文件表」），使用者口述時用的名字不一定跟正式名稱一樣，遇到「XX表」這種指代不明的名稱、且不確定是哪個代號時，要先跟使用者確認網址，不要用名稱長得像就猜代號直接連過去，猜錯了兩個不同試算表會被誤連在一起。`連結總表.md` 裡有登記已知的俗稱，可以先查那邊。
...
- `getPageCategory()`：依網址路徑判斷這頁屬於「遊戲類教學／營運活動教學／競品指南／Git使用教學／包鑽商城／其他文件／其他」哪個分類，回傳值是分類的**內部代號字串**（不一定跟 nav 顯示名稱完全一樣，例如「其他文件」這個分類，回傳的內部代號字串已改成 `'其他文件'`，但正則比對的關鍵字仍是舊檔名 `其他文件教學`，因為那是 `其他文件教學_索引.md` 這個檔名沒改的緣故）。正則比對路徑裡有沒有「遊戲」「營運/換皮/抽獎/直播/代言人/甘特」「競品」「文件格式/git」（不分大小寫，因為有的頁面檔名是 `Git...`、有的是全大寫 `如何使用GIT`）「包鑽商城」「BUG/友善遊戲/其他文件教學」，**加星星時會自動記錄當下頁面屬於哪個分類**。**判斷順序有陷阱**：`BUG`／`友善遊戲` 這類「其他文件」頁面的檢查必須排在「遊戲」的通用檢查**之前**，不然像 `友善遊戲.md` 這種檔名裡剛好含「遊戲」二字的頁面，會先被籠統的 `/\/遊戲/` 規則誤判成「遊戲類教學」——新增分類關鍵字時要注意跟既有規則會不會互相包含
...
**注意**：各分類的主題色（遊戲類教學紫／營運活動教學棕／競品指南綠／Git使用教學藍綠／包鑽商城紫紅／其他文件藍灰）**不是**寫在 extra.css，是直接寫在 `index.md` 各個卡片的 `style="color:#7c6a9c"` 這種 inline style 裡，逐個 div 手動上色的。六個分類顏色代碼：
- 遊戲類教學：`#7c6a9c`（底色 `#f2f0f5`）
- 營運活動教學：`#a8734f`（底色 `#f7f0e8`）
- 競品指南：`#6f8f76`（底色 `#eff4f0`）
- Git使用教學：`#2f8f83`（底色 `#e6f3f1`）
- 包鑽商城：`#9c5a8f`（底色 `#f5eef3`）
- 其他文件：`#5b7c99`（底色 `#eef2f6`）
...
4. **檢查檔名有沒有落在 `getPageCategory()` 的正則關鍵字裡**：如果新頁面的檔名不含任何現有分類關鍵字（例如原本 2026-08-14 新增的 `代言人更新流程.md` 不含「換皮」二字），會被誤判成「其他」分類，要在 `overrides/main.html` 的 `getPageCategory()` 正則加上新關鍵字
5. push
...
2. `overrides/main.html` 的 `DEFAULT_FAVORITES` 陣列加一筆，`category` 選 `遊戲類教學`／`營運活動教學`／`競品指南`／`Git使用教學`／`包鑽商城`／`其他文件`／`其他`
...
### 情景 6：改導航菜單 / 加新分類
`mkdocs.yml` 的 `nav:`。如果是全新的大分類（不是塞進現有六個），記得：
...
**分類只有單一頁面時，nav 可以扁平化**：不用硬寫成巢狀 `首頁:` 子選單，直接 `分類名稱: 檔名.md` 一行就好（例如曾經的 `其他文件教學: BUG回報方式.md`，現在分類改名叫「其他文件」了），跟 `包鑽商城: 包鑽商城.md`、`競品指南: 競品指南_索引.md` 是同樣的寫法慣例。之後這個分類如果又加了第二頁，才需要改回巢狀結構加 `首頁:` 條目（「其他文件」就是加了「友善遊戲」第二頁後改回巢狀的例子）。
