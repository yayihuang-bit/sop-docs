...
- `getPageCategory()`：依網址路徑判斷這頁屬於「遊戲類教學／營運活動教學／競品指南／Git使用教學／其他文件教學／其他」哪個分類（正則比對路徑裡有沒有「遊戲」「營運/換皮/抽獎/直播/代言人」「競品」「文件格式/git」（不分大小寫，因為有的頁面檔名是 `Git...`、有的是全大寫 `如何使用GIT`）「BUG」），**加星星時會自動記錄當下頁面屬於哪個分類**
