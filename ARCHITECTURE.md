...
- **各分類有自己的主題色**：遊戲類教學＝紫、營運活動教學＝棕、競品指南＝綠、Git使用教學＝藍綠、包鑽商城＝紫紅、其他文件教學＝藍灰，卡片邊框、按鈕、標題文字都統一套色
...
│   ├── 其他文件教學_索引.md／BUG回報方式.md／友善遊戲.md  # 其他文件教學分類（第 6 個大分類），有 2 頁以上後改回標準的「首頁＋子頁」巢狀結構
...
nav:                     # 見檔案內容，目前是「首頁 / 遊戲類教學 / 營運活動教學 / 競品指南 / Git使用教學 / 包鑽商城 / 其他文件教學」六大分類
...
- `getPageCategory()`：依網址路徑判斷這頁屬於「遊戲類教學／營運活動教學／競品指南／Git使用教學／包鑽商城／其他文件教學／其他」哪個分類（正則比對路徑裡有沒有「遊戲」「營運/換皮/抽獎/直播/代言人/甘特」「競品」「文件格式/git」（不分大小寫，因為有的頁面檔名是 `Git...`、有的是全大寫 `如何使用GIT`）「包鑽商城」「BUG/友善遊戲/其他文件教學」），**加星星時會自動記錄當下頁面屬於哪個分類**。**判斷順序有陷阱**：`BUG`／`友善遊戲` 這類「其他文件教學」頁面的檢查必須排在「遊戲」的通用檢查**之前**，不然像 `友善遊戲.md` 這種檔名裡剛好含「遊戲」二字的頁面，會先被籠統的 `/\/遊戲/` 規則誤判成「遊戲類教學」——新增分類關鍵字時要注意跟既有規則會不會互相包含
...
**注意**：各分類的主題色（遊戲類教學紫／營運活動教學棕／競品指南綠／Git使用教學藍綠／包鑽商城紫紅／其他文件教學藍灰）**不是**寫在 extra.css，是直接寫在 `index.md` 各個卡片的 `style="color:#7c6a9c"` 這種 inline style 裡，逐個 div 手動上色的。六個分類顏色代碼：
...
2. `overrides/main.html` 的 `DEFAULT_FAVORITES` 陣列加一筆，`category` 選 `遊戲類教學`／`營運活動教學`／`競品指南`／`Git使用教學`／`包鑽商城`／`其他文件教學`／`其他`
