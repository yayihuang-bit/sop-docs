...
│   ├── 其他文件教學_索引.md／BUG回報方式.md／友善遊戲.md  # 其他文件教學分類（第 5 個大分類），有 2 頁以上後改回標準的「首頁＋子頁」巢狀結構
...
- `getPageCategory()`：依網址路徑判斷這頁屬於「遊戲類教學／營運活動教學／競品指南／Git使用教學／其他文件教學／其他」哪個分類（正則比對路徑裡有沒有「遊戲」「營運/換皮/抽獎/直播/代言人」「競品」「文件格式/git」（不分大小寫，因為有的頁面檔名是 `Git...`、有的是全大寫 `如何使用GIT`）「BUG」），**加星星時會自動記錄當下頁面屬於哪個分類**。**判斷順序有陷阱**：`BUG`／`友善遊戲` 這類「其他文件教學」頁面的檢查必須排在「遊戲」的通用檢查**之前**，不然像 `友善遊戲.md` 這種檔名裡剛好含「遊戲」二字的頁面，會先被籠統的 `/\/遊戲/` 規則誤判成「遊戲類教學」——新增分類關鍵字時要注意跟既有規則會不會互相包含
