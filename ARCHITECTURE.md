...
│   ├── Git使用教學_索引.md／Git版本推送與衝突處理.md    # Git使用教學分類（第 4 個大分類）
...
- `getPageCategory()`：依網址路徑判斷這頁屬於「遊戲類教學／營運活動教學／競品指南／Git使用教學／其他文件教學／其他」哪個分類（正則比對路徑裡有沒有「遊戲」「營運/換皮/抽獎/直播」「競品」「文件格式/Git」「BUG」），**加星星時會自動記錄當下頁面屬於哪個分類**
