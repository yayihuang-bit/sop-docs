...
// 時間格式轉換：2026/4/10 00:00:00 → 2026-04-10 00:00:00
...
(async () => {
    ...
    for (let i = 0; i < docIds.length; i++) {
        ...
        // 擷取「APP-客服系統-定時廣播」區段
        const sectionText = extractSection(allText, 'APP-客服系統-定時廣播');
        if (!sectionText) {
            console.log('⚠️ 找不到「APP-客服系統-定時廣播」區段，跳過此文件');
            notify('⚠️ 定時廣播警告', `第 ${i + 1} 份文件找不到「APP-客服系統-定時廣播」區段，已跳過。\n\nhttps://docs.google.com/document/d/${docId}/edit`);
            failedDocIds.push(docId); failCount++;
            continue;
        }
        ...
        const timeRange = get('廣播起訖時間');
        const timeParts = timeRange.split(' - ');
        const intervalRaw = get('間隔');
        const extracted = {
            channel:   get('指定頻道'),
            content:   get('廣播內容'),
            ...
            count:     get('出現次數'),
            interval:  intervalRaw.replace(/[^0-9]/g, ''),
        };

        console.log(`📢 廣播內容: ${extracted.content}`);
        console.log(`📡 指定頻道: ${extracted.channel}`);
        console.log(`⏰ 起訖時間: ${extracted.startTime} ~ ${extracted.endTime}`);
        console.log(`🔢 出現次數: ${extracted.count}，間隔: ${extracted.interval} 分`);

        // 必填欄位驗證
        const missingFields = [];
        if (!extracted.content)   missingFields.push('廣播內容');
        if (!extracted.startTime) missingFields.push('廣播起訖時間(起)');
        if (!extracted.endTime)   missingFields.push('廣播起訖時間(訖)');
        if (!extracted.count)     missingFields.push('出現次數');
        if (!extracted.interval)  missingFields.push('間隔');
        ...
        // ── 前往後台填表 ──
        console.log('🚀 前往後台填定時廣播設定...');
        await page.goto(`${BACKSTAGE_URL}/index`);
        await page.waitForTimeout(2000);

        await page.getByRole('button', { name: '客服系統 <功能查詢>' }).click();
        await page.getByRole('navigation').getByRole('link', { name: '定時廣播' }).click();
        await page.waitForTimeout(2000);

        const frame = page.locator('#hbsIframe').contentFrame();
        await frame.locator('#BroadcastContent').waitFor({ state: 'visible', timeout: 30000 });

        // 廣播內容
        await frame.locator('#BroadcastContent').fill(extracted.content);

        // 起訖時間：點擊後關閉日期選擇器，再用鍵盤輸入
        const activeTimeStr = `${formatTime(extracted.startTime)} - ${formatTime(extracted.endTime)}`;
        await frame.locator('#ActiveTime').click();
        await page.waitForTimeout(300);
        await page.keyboard.press('Escape');
        await page.waitForTimeout(300);
        await frame.locator('#ActiveTime').selectText();
        await page.keyboard.type(activeTimeStr);
        await page.keyboard.press('Tab');
        await page.waitForTimeout(500);

        // 出現次數
        await frame.locator('#FreqNumber').fill(extracted.count || '');

        // 間隔(分)
        await frame.locator('#IntervalNumber').fill(extracted.interval || '');
        ...
        await frame.getByRole('button', { name: '確定新增' }).click();
        await page.waitForTimeout(2000);
        if (!docSuccess) {
            failedDocIds.push(docId); failCount++;
            console.log(`❌ 第 ${i + 1} 份定時廣播設定失敗`);
        } else {
            successDocIds.push(docId);
            console.log(`✅ 第 ${i + 1} 份定時廣播設定完成！`);
        }
