...
(async () => {
    ...
    for (let i = 0; i < docIds.length; i++) {
        ...
        // 擷取「官網設定-文章管理-AI自動化用」區段
        const sectionText = extractSection(allText, '官網設定-文章管理-AI自動化用');
        if (!sectionText) {
            console.log('⚠️ 找不到「官網設定-文章管理-AI自動化用」區段，跳過此文件');
            notify('⚠️ 官網文章警告', `第 ${i + 1} 份文件找不到「官網設定-文章管理-AI自動化用」區段，已跳過。\n\nhttps://docs.google.com/document/d/${docId}/edit`);
            failedDocIds.push(docId); failCount++;
            continue;
        }
        ...
        const contentMatch = sectionText.match(/(?:內文|內容)[：:]\s*\n([\s\S]*?)\n*上架時間[：:]/);
